...
for k, key in enumerate(['a', 'c', 'g', 'u','.']):
    aadict[key] = aadict[aa[k]]
aadict['N'] = len(aa)
aadict['S'] = len(aa)
aadict['Y'] = len(aa)
# gives the same number to uppercase and lowercase characters, and gap can be either '-' or '.'
# some (few, 12 out of 5218) sequences have letters like N, S or Y, not corresponding to any of the four basis + gap

curr_int = np.int16
curr_float = np.float32


# gives the same color to Watson-Crick canonical base pairs (A-U in red, C-G in blue), maybe one can do better
def aa_color(letter):
    if letter in ['C', 'G']:
        return 'blue'
    elif letter in  ['A', 'U']:
...
def load_RNA_MSA(filename, with_labels=False, remove_insertions = True, drop_duplicates=True):
    count = 0
    current_seq = ''
    all_seqs = []
    if with_labels:
